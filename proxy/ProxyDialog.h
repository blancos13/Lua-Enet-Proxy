...
#include <string>
using namespace std;
enum LabelStyles { LABEL_BIG, LABEL_SMALL };

enum SpacerTypes { SPACER_BIG, SPACER_SMALL };

enum CheckboxTypes { CHECKBOX_NOT_SELECTED, CHECKBOX_SELECTED };
class Dialog {
   public:
    string dialogstr = "";
    void addSpacer(SpacerTypes type);
    void addLabelWithIcon(std::string text, int tileid, LabelStyles type);
    void addLabelWithIconButton(std::string text, int tileid, string buttontext);
